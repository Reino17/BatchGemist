	              ...
	              //div[
	                ...
	                position^(^):let $a:^=doc^(./video/@src^|^|'/manifest.mpd'^)//@mediaPresentationDuration return {
	                  'name':concat^(
	                    'Omrop Fryslƒn: '^,
	                    replace^(
	                      if ^(./div[@class^='vjs-title']^) then
	                        ./div[@class^='vjs-title']
	                      else
	                        //div[@class^='node-content-wrapper']//h3^,
	                      '[^&quot^;^&apos^;]'^,
	                      ''''''
	                    ^)^,
	                    $date
