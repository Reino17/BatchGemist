...
IF NOT "%url: =%"=="%url%" (
	...
	FOR /F "delims=" %%A IN ('^"%xidel% "%url%" --xquery "declare function local:extract($arg){[for $x at $i in $arg order by extract($x/@data-label,'(\d+)p',1) count $i let $a:=if (contains($x/@src,'content-ip')) then x:request({'post':serialize-json([{'file':string($x/@src)}]),'url':'https://nos.nl/video/resolve/'})//file else $x/@src return system(x'cmd /c "FFMpeg\ffmpeg-N-87867-g0655810-win32-static_legacy.exe" -i {$a} 2>&amp;1') ! {'format':'mp4-'||$i,'extension':'mp4','resolution':extract(.,'Video:.+, (\d+x\d+)',1),'vbitrate':replace(.,'.+Video:.+?(\d+) kb.+','v:$1k','s'),'abitrate':replace(.,'.+Audio:.+?(\d+) kb.+','a:$1k','s'),'url':$a}]}; if (//div[@class='video-play']) then videos:=[//div[@class='video-play']/a/doc(@href)/{position():{'name':concat('NOS: ',replace(//h1,'[&quot;&apos;]',''''''),replace(//@datetime,'(\d+)-(\d+)-(\d+).+',' ($3$2$1)')),'formats':local:extract(//source)}}] else (name:=concat('NOS: ',replace(//h1,'[&quot;&apos;]',''''''),replace(//@datetime,'(\d+)-(\d+)-(\d+).+',' ($3$2$1)')),formats:=local:extract(//source))" --output-encoding^=oem --output-format^=cmd^"') DO %%A
