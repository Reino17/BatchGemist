...
IF NOT "%url: =%"=="%url%" (
	...
	FOR /F "delims=" %%A IN ('^"%xidel% "%url%" --xquery "name:=//h1||replace('%date%','.+?(\d+)-(\d+)-(\d+)',': Livestream ($1$2$3)'),let $a:=//video/x:request({'post':serialize-json({'stream':string(@data-stream)}),'url':@data-path})/json/json(substring-before(url,'p&amp;callback')) return formats:=[{'format':'hls-master','extension':'m3u8','url':$a},for $x in tail(tokenize(extract(unparsed-text($a),'(#EXT-X-STREAM-INF.+m3u8$)',1,'ms'),'#EXT-X-STREAM-INF:')) ! {'format':'hls-'||extract(.,'BANDWIDTH=(\d+)\d{3}',1),'extension':'m3u8','resolution':extract(.,'RESOLUTION=([\dx]+),',1),'vbitrate':extract(.,'video=(\d+)\d{3}',1) ! (if (.) then concat('v:',.,'k') else ''),'abitrate':replace(.,'.+audio.+?(\d+)\d{3}.+','a:$1k','s'),'url':resolve-uri('.',$a)||extract(.,'(.+m3u8)',1)} order by $x/format return $x]" --output-encoding^=oem --output-format^=cmd^"') DO %%A
) ELSE IF NOT "%url:nos.nl=%"=="%url%" (
	FOR /F "delims=" %%A IN ('^"%xidel% "%url%" --xquery "declare function local:extract($arg){[for $x at $i in $arg order by extract($x/@data-label,'(\d+)p',1) count $i let $a:=if (contains($x/@src,'content-ip')) then x:request({'post':serialize-json([{'file':string($x/@src)}]),'url':'https://nos.nl/video/resolve/'})//file else $x/@src return system(x'cmd /c %ffmpeg% -i {$a} 2>&amp;1') ! {'format':'mp4-'||$i,'extension':'mp4','resolution':extract(.,'Video:.+, (\d+x\d+)',1),'vbitrate':replace(.,'.+Video:.+?(\d+) kb.+','v:$1k','s'),'abitrate':replace(.,'.+Audio:.+?(\d+) kb.+','a:$1k','s'),'url':$a}]}; if (//div[@class='video-play']) then videos:=[//div[@class='video-play']/a/doc(@href)/{position():{'name':concat('NOS: ',replace(//h1,'[&quot;&apos;]',''''''),replace(//@datetime,'(\d+)-(\d+)-(\d+).+',' ($3$2$1)')),'formats':local:extract(//source)}}] else (name:=concat('NOS: ',replace(//h1,'[&quot;&apos;]',''''''),replace(//@datetime,'(\d+)-(\d+)-(\d+).+',' ($3$2$1)')),formats:=local:extract(//source))" --output-encoding^=oem --output-format^=cmd^"') DO %%A
	...
	FOR /F "delims=" %%A IN ('^"%xidel% "%url%" --xquery "videos:=if (//@data-mrss) then if (count(//@data-mrss)=1) then [{'1':{'name':'Comedy Central: '||replace(//div[@class='episode_information']/h2,'[&quot;&apos;]',''''''),'prid':extract(//@data-mrss,'(local.+)',1),'goto':'MTV'}}] else [//li[@data-mrss]/{position():{'name':'Comedy Central: '||replace(if (@data-franchise) then concat(@data-franchise,' - ',.//img/@alt) else .//img/@alt,'[&quot;&apos;]',''''''),'prid':extract(@data-mrss,'(local.+)',1),'goto':'MTV'}}] else [{'1':{'name':'Comedy Central: '||replace(//meta[@property='og:title']/@content,'[&quot;&apos;]',''''''),'prid':json(//script/extract(.,'playObject = (.+?);',1,'s')[.])/concat(type,'-',token),'goto':'MTV'}}]" --output-encoding^=oem --output-format^=cmd^"') DO %%A
) ELSE IF NOT "%url:nl.funnyclips.cc=%"=="%url%" (
	FOR /F "delims=" %%A IN ('^"%xidel% "%url%" --xquery "videos:=[{'1':{'name':'FunnyClips: '||replace(concat(//h3[@class='franchise_title']/text()[1],' - ',//h2[@class='title']),'[&quot;&apos;]',''''''),'prid'://script/extract(.,'http.+(local.+)''',1)[.],'goto':'MTV'}}]" --output-encoding^=oem --output-format^=cmd^"') DO %%A
...
:MTV
FOR /F "delims=" %%A IN ('^"%xidel% "http://api.mtvnn.com/v2/mrss.xml?uri=mgid:sensei:video:mtvnn.com:%prid%" --xquery "name:=replace('%name:^=%','[&quot;&apos;]','''''')||replace(//pubDate,'(\d+)-(\d+)-(\d+).+',' ($3$2$1)'),t:=//media:content/@duration,duration:=$t * dayTimeDuration('PT1S') + time('00:00:00')" -f "//media:content/@url" -e "formats:=[//rendition/{'format':'mp4-'||position(),'extension':'mp4','resolution':concat(@width,'x',@height),'vbitrate':concat('v:',@bitrate,'k'),'url':src}]" --output-encoding^=oem --output-format^=cmd^"') DO %%A
